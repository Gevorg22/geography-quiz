...
describe("GameOver", () => {
  it("отображает заголовок «Игра завершена!»", () => {
    render(<GameOver stats={baseStats} totalPoints={0} maxPoints={30} onRestart={vi.fn()} />);
  ...
  it("отображает количество стран угаданных с первой попытки", () => {
    ...
    render(<GameOver stats={stats} totalPoints={15} maxPoints={30} onRestart={vi.fn()} />);
  ...
  it("отображает количество не угаданных стран", () => {
    ...
    render(<GameOver stats={stats} totalPoints={0} maxPoints={30} onRestart={vi.fn()} />);
  ...
  it("отображает итоговый счёт и максимум", () => {
    render(<GameOver stats={baseStats} totalPoints={12} maxPoints={30} onRestart={vi.fn()} />);
    expect(screen.getByText("12")).toBeInTheDocument();
    expect(screen.getByText(/\/\s*30 очков/i)).toBeInTheDocument();
  });

  it("показывает рейтинг 🏆 при доле ≥ 90%", () => {
    render(<GameOver stats={baseStats} totalPoints={27} maxPoints={30} onRestart={vi.fn()} />);
    expect(screen.getByText(/идеально/i)).toBeInTheDocument();
  });

  it("показывает рейтинг 🥇 при доле от 70% до 89%", () => {
    render(<GameOver stats={baseStats} totalPoints={21} maxPoints={30} onRestart={vi.fn()} />);
  ...
  it("отображает очки за первую попытку корректно", () => {
    ...
    render(<GameOver stats={stats} totalPoints={9} maxPoints={30} onRestart={vi.fn()} />);
    expect(screen.getByText(expected)).toBeInTheDocument();
  });

  it("вызывает onRestart при нажатии кнопки", async () => {
    const onRestart = vi.fn();
    render(<GameOver stats={baseStats} totalPoints={0} maxPoints={30} onRestart={onRestart} />);
    await userEvent.click(screen.getByRole("button", { name: /ещё раз/i }));
    expect(onRestart).toHaveBeenCalledOnce();
  });

  it("показывает факт если передан", () => {
    render(<GameOver stats={baseStats} totalPoints={0} maxPoints={30} onRestart={vi.fn()} fact="Интересный факт" />);
    expect(screen.getByText("Интересный факт")).toBeInTheDocument();
  });
});
